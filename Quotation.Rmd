...
`substitute()` provides a sort of automatic unquoting for any symbol that is bound to a value. However, making use of this behaviour can make for hard to read code, because for example, taken out of context, you can't tell if the goal of `substitute(x + y)` is to replace `x`, or, `y`, or both. If you do want to use `substitute()` in this way, I recomend that you use the 2nd argument to make it clear that is your goal:
...
And you want to convert it into an expression like `10 + (5 * x1) + (-4 * x2)`. The first thing we need is to turn the names of the character vector into a list of symbols. `rlang::syms()` is designed precisely for this case:
...
We'll need to generated a call with multiple missing arguments. Fortunately is easy with `rep()` and `missing_arg()`. Once we have those arguments, we can unquote-splice them into a call:
...
Here `x` is a pronoun. As with `.` in pipelines and `.x` and `.y` in purrr functioanls, `x` doesn't represent a single concrete value, but is instead a placeholder that varies over the range of the plot. Functions, like `curve()`, that use an expression containing a pronoun are known as __anaphoric__ functions[^anaphora].
...
1.  Implement `partition_rows()` which, like `partition_cols()`, returns two
...
*   The second problem is like unquoting on the LHS of `=`: rather than 
    ...
    Note the use of `:=` (pronounced colon-equals) rather than `=`. Unforunately 
    ...
    `:=` is like a vestigal organ: it's recognised by R's parser, but it doesn't
